...
type TradeOrderStatus = "COMPLETED" | "SETTLING";
...
const sideLabel: Record<TradeOrderSide, string> = {
  BUY: "买",
  SELL: "卖",
};

const statusLabel: Record<TradeOrderStatus, string> = {
  COMPLETED: "已成交",
  SETTLING: "结算中",
};

const mockRows: TradeOrderRecord[] = Array.from({ length: 10 }, (_, index) => ({
  id: `TO${String(index + 1).padStart(8, "0")}`,
  address: "0x1000...00X1",
  side: index % 3 === 0 ? "BUY" : "SELL",
  pair: "rFUIDL/USDC",
  avgPrice: "1.0008",
  quantity: "1,000.01",
  totalAmount: "1,000.81 USDC",
  hash: "0xd850...8438",
  status: index < 2 ? "SETTLING" : "COMPLETED",
...
function StatusTag({ status }: { status: TradeOrderStatus }) {
  const tone =
    status === "SETTLING"
...
export function TradeOrderPage() {
  ...
  const [status, setStatus] = useState<"全部" | TradeOrderStatus>("全部");
  const [side, setSide] = useState<"全部" | TradeOrderSide>("全部");
  const [pair, setPair] = useState("全部");
  ...
  const rows = useMemo(
    () =>
      mockRows.filter((row) => {
        ...
        const matchAddress = address.trim() === "" || row.address.includes(address.trim());
        return matchStatus && matchSide && matchPair && matchAddress;
      }),
    [address, pair, side, status]
  );
    ...
    <div className="space-y-4">
      <div className="rounded-[8px] border border-[#eceff5] bg-white px-5 py-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select
            value={side}
            onChange={(event) => setSide(event.target.value as "全部" | TradeOrderSide)}
          ...
          <select
            value={status}
            onChange={(event) => setStatus(event.target.value as "全部" | TradeOrderStatus)}
      ...
      <div className="rounded-[8px] border border-[#eceff5] bg-white">
        <div className="border-b border-[#eceff5] px-5 py-3 text-[13px] font-medium text-slate-700">成交订单</div>
        <div className="overflow-x-auto">
          <table className="w-full table-fixed border-collapse text-[13px]">
            <thead className="bg-[#fafbfd] text-left text-slate-500">
              <tr>
                {["成交 ID", "交易地址", "买卖", "交易对", "成交均价", "成交数量", "成交总额", "交易 HASH", "状态", "创建时间"].map((title) => (
