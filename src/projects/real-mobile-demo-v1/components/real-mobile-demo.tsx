...
import {
  ...
  type TableRow,
} from "@/projects/real-mobile-demo-v1/lib/real-mobile-data";
...
export function RealMobileDemo({ slug }: RealMobileDemoProps) {
  ...
  const content = (() => {
    if (routePath === "/" || routePath === "/trade") return <TradePage connected={isConnected} />;
    ...
    if (routePath === "/buy" || routePath === "/sell" || routePath === "/redeem" || routePath === "/claim") {
  ...
  const bottomActive = routePath === "/orders" || routePath === "/orders/detail" ? "orders" : "market";
  const showBottom = ["/", "/trade", "/market", "/orders", "/portfolio", "/invite", "/stake"].includes(routePath);
    ...
    <>
      <header className={styles.header}>
        <div className={styles.headerLeft}>
          <Link href={toDemoPath("/")} aria-label="Real mobile home">
...
function TradePage({ connected = true, forceEmpty = false }: { connected?: boolean; forceEmpty?: boolean }) {
  ...
  const products = forceEmpty
    ? []
    : realMobileProducts.filter((product) => `${product.symbol}${product.name}${product.publisher}`.toLowerCase().includes(query.toLowerCase()));
        ...
        {products.length > 0 ? (
          <div className={styles.cardList}>
            {products.map((product, index) => (
              <AssetCard product={product} defaultExpanded={index === 1} key={product.id} />
            ))}
          </div>
        ) : (
...
function AssetCard({ product, defaultExpanded = false }: { product: RealMobileProduct; defaultExpanded?: boolean }) {
  ...
  const disabled = product.status !== "selling";
  ...
  return (
    <article className={styles.assetCard}>
      ...
      <div className={styles.progressPanel}>
        ...
        <button className={styles.poolRow} type="button" aria-expanded={expanded} onClick={() => setExpanded((isExpanded) => !isExpanded)}>
          <span>认购池</span>
        ...
        {expanded ? (
          <div className={styles.expandedDetail}>
            <InfoLine label="发行机构：" value={product.publisherEn} />
            <InfoLine label="最小购买数量：" value={product.minimum} />
            <InfoLine label="发行日期：" value={product.issueDate} />
            <InfoLine label="到期日期：" value={product.expireDate} />
          </div>
        ) : null}
      </div>

      <div className={styles.actions}>
        <Link className={styles.outlineButton} href={toDemoPath(`/coinDetail?coin=${product.id}&tab1=detail`)}>
          详情
        </Link>
        {disabled ? (
    ...
    <>
      ...
      <section className={styles.detailHero}>
        ...
        <div className={styles.metricGrid}>
          <Metric label="最优价格" value={product.priceUnit} />
          <Metric label="综合收益构成" value={`${product.apr}%`} />
          <Metric label="累计奖励" value="128.72" />
          <Metric label="未领取收益" value="42.18" />
        </div>
      </section>
      ...
      {activeTab === "stake" ? <StakeOperatePanel product={product} compact /> : null}
  ...
  return (
    <>
      <section className={styles.surface}>
        <SectionTitle title="综合收益构成" subtitle="RWA 底层资产收益加上平台补贴/收益，iREAL 积分作为平台补贴收益。" />
      ...
      <section className={styles.surface}>
        <SectionTitle title="更多信息" subtitle={product.description} />
        <InfoLine label="金融发行方" value={product.publisher} />
        <InfoLine label="到期时间" value={product.expireDate} />
        <InfoLine label="单次起购数量" value={product.minimum} />
        <InfoLine label="链上市值" value={product.marketCap} />
        <InfoLine label="总供应量" value={product.totalLabel} />
        <InfoLine label="债券周期" value={product.cycle} />
...
function InlineTradePanel({ product, tab2 }: { product: RealMobileProduct; tab2: string }) {
  const mode = tab2 === "sell" ? "sell" : "buy";
  return (
    <section className={styles.formPanel}>
      <div className={styles.actions}>
        <Link className={mode === "buy" ? styles.goldButton : styles.outlineButton} href={toDemoPath(`/coinDetail?coin=${product.id}&tab1=trade&tab2=buy`)}>
          购买
        </Link>
        <Link className={mode === "sell" ? styles.goldButton : styles.outlineButton} href={toDemoPath(`/coinDetail?coin=${product.id}&tab1=trade&tab2=sell`)}>
          卖出
        </Link>
      </div>
      <AmountField label={mode === "buy" ? "支付金额" : "卖出数量"} value="0.00" unit={mode === "buy" ? "USDC" : product.symbol} />
      <AmountField label="预计获得" value="0.00" unit={mode === "buy" ? product.symbol : "USDC"} />
      <InfoLine label="GAS 费" value="0.005 BNB" />
      <Link className={styles.fullGoldButton} href={toDemoPath(mode === "buy" ? "/buy" : "/sell")}>
        {mode === "buy" ? "购买" : "卖出"}
      </Link>
    </section>
  );
...
function StakeOperatePanel({ product, compact = false }: { product: RealMobileProduct; compact?: boolean }) {
  return (
    <section className={styles.formPanel}>
      ...
      <AmountField label="预计获得" value="0.00" unit="R Token" />
      <InfoLine label="兑换比例" value={`1 ${product.symbol} = 1 R Token`} />
      <InfoLine label="参考年化" value={`${product.apr}%`} />
      <InfoLine label="预计月收益" value="325 R Token" />
                ...
                {expanded ? (
                  <>
                    ...
                    <div className={styles.portfolioHoldingActions}>
                      ...
                      <Link className={styles.portfolioOutlineAction} href={toDemoPath(`/stake/operate?coin=${asset.productId}`)}>质押</Link>
                      <Link className={styles.portfolioGoldAction} href={toDemoPath(`/buy?coin=${asset.productId}`)}>购买</Link>
                    </div>
                  </>
                ) : null}
              </article>
      ...
      <aside className={styles.walletDrawer}>
        {status === "select" ? (
          <>
            <div className={styles.walletDrawerHead}>
              <CircleHelp size={20} />
              <h1>连接钱包</h1>
              <Link href={toDemoPath("/trade")} aria-label="关闭连接钱包">
                <X size={24} />
              </Link>
            </div>
        ...
        {status === "success" ? (
          <div className={styles.walletProgress}>
            ...
            <Link className={styles.fullGoldButton} href={toDemoPath("/trade")}>进入市场</Link>
          </div>
        ) : null}
      </aside>
    </>
  ...
  return (
    <>
      ...
      <section className={styles.resultPanel}>
        ...
        <h1>提交成功</h1>
        <p>交易已进入订单列表，您可以继续查看订单状态或返回市场。</p>
      </section>
      <div className={styles.actions}>
        <Link className={styles.outlineButton} href={toDemoPath("/trade")}>返回市场</Link>
...
function BottomTab({ active }: { active: "market" | "orders" }) {
  return (
