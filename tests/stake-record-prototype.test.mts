...
test("质押记录原型使用截图中的摘要与时间字段", () => {
  assert.deepEqual(stakeRecordColumns, ["Staking地址", "邮箱", "Staking币种", "数量", "Hash", "质押时间", "赎回时间", "Staking时长"]);
  ...
  assert.deepEqual(stakeRecordRows[0], {
    ...
    redeemedAt: "2026-10-12",
    stakingDuration: "100分钟",
  });
});
...
test("质押记录页面显示筛选文案、统计卡与总条数", () => {
  ...
  for (const label of ["RWA资产", "开始时间", "结束时间", "邮箱", "展开", "Staking总额", "Staking地址数", "共 119 条", "row.redeemedAt", "row.stakingDuration"]) {
